...
class TransformerEncoderLayerRPR(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, er_len=None, reduce_qk=False, device='cuda:0'):
        ...
        self.self_attn = MultiheadAttentionRPR(d_model, nhead, dropout=dropout, er_len=er_len, reduce_qk=reduce_qk, device=device)
...
class MultiheadAttentionRPR(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, er_len=None, reduce_qk=False, device='cuda:0'):
        ...
        self.skew_mask = (torch.triu(torch.ones(er_len, er_len).to(device)) == 1).float().flip(0)

        if self._qkv_same_embed_dim is False:
            if self.reduce_qk:
                self.q_proj_weight = Parameter(torch.Tensor(int(embed_dim / 2), embed_dim))
                self.k_proj_weight = Parameter(torch.Tensor(int(embed_dim / 2), self.kdim))
        ...
        if(er_len is not None):
            if self.reduce_qk:
                self.Er = Parameter(torch.rand((er_len, int(self.head_dim / 2)), dtype=torch.float32))
            else:
    ...
    def forward(self, query, key, value, key_padding_mask=None,
        ...
        if hasattr(self, '_qkv_same_embed_dim') and self._qkv_same_embed_dim is False:
            ...
            return multi_head_attention_forward_rpr(
                ...
                v_proj_weight=self.v_proj_weight, rpr_mat=self.Er, reduce_qk=self.reduce_qk, skew_mask=self.skew_mask)
        else:
            ...
            return multi_head_attention_forward_rpr(
                ...
                training=self.training,
                key_padding_mask=key_padding_mask, need_weights=need_weights,
                attn_mask=attn_mask, rpr_mat=self.Er, reduce_qk=self.reduce_qk, skew_mask=self.skew_mask)
...
def multi_head_attention_forward_rpr(query,                       # type: Tensor
    ...
    if use_separate_proj_weight is not True:
        ...
        if in_proj_bias is not None:
            if reduce_qk:
                q = linear(query, q_proj_weight_non_opt, in_proj_bias[0:int(embed_dim/2)])
                k = linear(key, k_proj_weight_non_opt, in_proj_bias[int(embed_dim/2):embed_dim])
    ...
    if reduce_qk:
        q = q.contiguous().view(tgt_len, bsz * num_heads, int(head_dim / 2)).transpose(0, 1)
        if k is not None:
            k = k.contiguous().view(-1, bsz * num_heads, int(head_dim / 2)).transpose(0, 1)
        if v is not None:
...
def _skew(qe, skew_mask=None):
    ...
    sz = qe.shape[1]
    # If mask is generated on the fly performance might degrade
    if skew_mask is None:
        mask = (torch.triu(torch.ones(sz, sz).to(qe.device)) == 1).float().flip(0)
    else:
        mask = skew_mask[..., skew_mask.shape[0] - sz:, :sz]
